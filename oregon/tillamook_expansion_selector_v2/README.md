...
SLIDO polygon/group novelty is keyed by `UNIQUE_ID`, with `OBJECTID`/feature-id fallback. `REF_ID_COD` is **not** used as polygon identity because it is a source/reference code that can repeat across many deposits. The selector now fails fast when high/moderate SLIDO records collapse into suspiciously few unique keys.

If a v1 selection shows `new_positive_polygon_count = 0` for every candidate, discard that proposal and rerun v2 before downloading.
