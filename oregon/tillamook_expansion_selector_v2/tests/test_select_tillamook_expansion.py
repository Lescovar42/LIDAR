...
class ExpansionSelectorTests(unittest.TestCase):
    ...
    def test_feature_key_prefers_unique_id_over_repeated_reference_code(self):
        feature = {
            "properties": {
                "REF_ID_COD": "CalhNC2020a",
                "UNIQUE_ID": "LS-000123",
                "OBJECTID": 42,
            }
        }
        self.assertEqual(feature_key(feature, 0), "LS-000123")

    def test_feature_key_falls_back_to_objectid_not_reference_code(self):
        feature = {"properties": {"REF_ID_COD": "CalhNC2020a", "OBJECTID": 77}}
        self.assertEqual(feature_key(feature, 0), "77")

    def test_identity_sanity_check_rejects_collapsed_keys(self):
        polys = [self.poly("same", "high", (-123.70 + i * .001, 45.50, -123.699 + i * .001, 45.501)) for i in range(30)]
        with self.assertRaises(ValueError):
            validate_polygon_identity(polys)
